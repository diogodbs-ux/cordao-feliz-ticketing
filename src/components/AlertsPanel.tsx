...
export default function AlertsPanel() {
  ...
  const alerts = useMemo(() => {
    ...
    const maxCapacity = config.capacidadeMaxima;
    const occupancy = (totalAtendidos / maxCapacity) * 100;
    if (config.alertaCapacidade90 && occupancy > 90) {
      list.push({
        id: 'cap-critical',
        type: 'capacity',
        severity: 'critical',
    ...
    } else if (config.alertaCapacidade75 && occupancy > 75) {
      list.push({
        id: 'cap-warning',
        type: 'capacity',
        severity: 'warning',
  ...
  }, [todayCheckins, grupos, gruposHoje, stats, config]);
